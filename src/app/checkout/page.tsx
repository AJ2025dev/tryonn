    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-6" style={{ backgroundColor: "#FDFCFA" }}>
        ...
        <div className="max-w-md w-full text-center">
          <div className="w-16 h-16 rounded-full border-2 border-green-700 flex items-center justify-center mx-auto mb-6">
            <svg xmlns="http://www.w3.org/2000/svg" className="w-8 h-8 text-green-700" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
          </div>
          ...
          <Link href="/" className="inline-block w-full py-3.5 text-xs tracking-[0.2em] uppercase bg-stone-900 text-white hover:bg-stone-800 transition-colors" style={{ fontFamily: "'Outfit', sans-serif" }}>
            Continue Shopping
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen" style={{ backgroundColor: "#FDFCFA" }}>
      ...
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-sm border-b border-stone-100">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <Link href="/cart" className="text-2xl font-light tracking-wide text-stone-900" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Checkout</Link>
          <Link href="/cart" className="text-xs tracking-[0.1em] uppercase text-stone-400 hover:text-stone-900 transition-colors" style={{ fontFamily: "'Outfit', sans-serif" }}>Back to Bag</Link>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6 py-10" style={{ fontFamily: "'Outfit', sans-serif" }}>
        ...
        <div className="grid md:grid-cols-3 gap-12">
          <div className="md:col-span-2 space-y-10">
            {/* Contact */}
            <div>
              <h2 className="text-xs tracking-[0.2em] uppercase text-stone-900 font-medium mb-5">Contact Details</h2>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-xs text-stone-500 mb-1.5">First Name *</label><input value={form.firstName} onChange={e => updateField("firstName", e.target.value)} className={inputClass} placeholder="John" /></div>
                <div><label className="block text-xs text-stone-500 mb-1.5">Last Name</label><input value={form.lastName} onChange={e => updateField("lastName", e.target.value)} className={inputClass} placeholder="Doe" /></div>
              </div>
              <div className="mt-4"><label className="block text-xs text-stone-500 mb-1.5">Phone Number *</label><input value={form.phone} onChange={e => updateField("phone", e.target.value.replace(/\D/g, "").slice(0, 10))} className={inputClass} placeholder="9876543210" /></div>
            </div>

            {/* Address */}
            <div>
              <h2 className="text-xs tracking-[0.2em] uppercase text-stone-900 font-medium mb-5">Delivery Address</h2>
              <div className="space-y-4">
                <div><label className="block text-xs text-stone-500 mb-1.5">House / Flat No *</label><input value={form.houseNo} onChange={e => updateField("houseNo", e.target.value)} className={inputClass} placeholder="Flat 401, Tower B" /></div>
                <div><label className="block text-xs text-stone-500 mb-1.5">Address Line 1 *</label><input value={form.address1} onChange={e => updateField("address1", e.target.value)} className={inputClass} placeholder="Street name, Area" /></div>
                <div><label className="block text-xs text-stone-500 mb-1.5">Address Line 2</label><input value={form.address2} onChange={e => updateField("address2", e.target.value)} className={inputClass} placeholder="Colony, Locality" /></div>
                <div><label className="block text-xs text-stone-500 mb-1.5">Landmark</label><input value={form.landmark} onChange={e => updateField("landmark", e.target.value)} className={inputClass} placeholder="Near Metro Station" /></div>
                <div className="grid grid-cols-3 gap-4">
                  <div><label className="block text-xs text-stone-500 mb-1.5">City *</label><input value={form.city} onChange={e => updateField("city", e.target.value)} className={inputClass} placeholder="Hyderabad" /></div>
                  <div><label className="block text-xs text-stone-500 mb-1.5">State *</label><input value={form.state} onChange={e => updateField("state", e.target.value)} className={inputClass} placeholder="Telangana" /></div>
                  <div><label className="block text-xs text-stone-500 mb-1.5">Pincode *</label><input value={form.pincode} onChange={e => updateField("pincode", e.target.value.replace(/\D/g, "").slice(0, 6))} className={inputClass} placeholder="500081" /></div>
                </div>
              </div>
            </div>
            ...
            {/* Payment */}
            <div>
              <h2 className="text-xs tracking-[0.2em] uppercase text-stone-900 font-medium mb-5">Payment Method</h2>
              <div className="space-y-3">
                <label className={`flex items-center gap-4 p-4 border cursor-pointer transition-all ${form.paymentMethod === "cod" ? "border-stone-900 bg-stone-50/50" : "border-stone-200 hover:border-stone-400"}`}>
                  <input type="radio" name="payment" checked={form.paymentMethod === "cod"} onChange={() => updateField("paymentMethod", "cod")} className="w-4 h-4 accent-stone-900" />
                  <div><p className="text-sm text-stone-900">Cash on Delivery</p><p className="text-[11px] text-stone-400">Pay when your order arrives</p></div>
                </label>
          ...
          <div className="md:sticky md:top-24 h-fit">
            <div className="border border-stone-200 p-8">
              <h2 className="text-xs tracking-[0.2em] uppercase text-stone-900 font-medium mb-6">Your Order</h2>
              <div className="space-y-4 mb-6">
                {cart.map(item => (
                  <div key={item.variantId} className="flex gap-3 items-center">
                    <div className="w-14 h-18 flex-shrink-0 overflow-hidden" style={{ backgroundColor: "#F0EBE3" }}>
                      <img src={item.image || "https://placehold.co/56x72/F0EBE3/8B6F4E"} alt="" className="w-full h-full object-cover" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs text-stone-900 font-medium truncate">{item.name}</p>
                      <p className="text-[11px] text-stone-400">{item.size} × {item.quantity}</p>
                    </div>
                    <p className="text-xs text-stone-900 font-medium flex-shrink-0">{fmt(item.price * item.quantity)}</p>
                  </div>
                ))}
              </div>
