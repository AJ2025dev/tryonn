...
export async function POST(req: NextRequest) {
  try {
    ...
    // 2. Call Claude API to generate DesignSpec
    const designSpec = await generateDesignSpec(brief);

    // 3. Determine logo URL
    ...
    const { data: merchant, error: merchantErr } = await supabase
      .from("merchants")
      .insert({
        email: `${brief.storeUrl}@appi-fy.ai`,
    ...
    // 6. Save MerchantBrief
    await supabase.from("merchant_briefs").insert({
      merchant_id: merchant.id,
      brief_json: brief,
      status: "approved",
    });
    ...
    // 7. Save DesignSpec
    await supabase.from("design_specs").insert({
      merchant_id: merchant.id,
...
Generate a JSON design spec with these EXACT fields. Choose colors, fonts, and text that perfectly match this brand's category, style, and audience. Be specific and creative.

Respond with ONLY valid JSON, no other text:
{
  "primaryColor": "#hex (main brand color)",
  "secondaryColor": "#hex (complementary color)",
  "accentColor": "#hex (highlight color for sales badges)",
  "backgroundColor": "#hex (page background)",
  "textColor": "#hex (main text color)",
  "textOnPrimary": "#hex (text on primary color background)",
  "fontFamily": "font name (choose from: Cormorant Garamond, Playfair Display, Libre Baskerville, Montserrat, Poppins, Raleway, Lora, DM Sans, Space Grotesk, Sora)",
  "bodyFont": "font name for body text",
  "tagline": "short compelling tagline (max 8 words)",
